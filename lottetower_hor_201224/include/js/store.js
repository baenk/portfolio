...
function setInitConfig(p_config,p_arr_store){
    ...
    for(i = 0; i < p_arr_store.length; i++){
        if(p_arr_store[i].TYPE == "STORE"){
            if( p_arr_store[i].BUILDING != "SHPM" ){
...
function setContentsDir(p_dir){
    ...
    var i_left = 0;
    var i_width = gl_store_box_width * 5;
...
function getCalPos(p_num){
    ...
    var i_left = Math.floor(p_num % 5) * gl_store_box_width;
    var i_top = Math.floor((p_num % gl_paging_conf.list_cnt) / 5) * gl_store_box_height;

    i_left = i_left + (gl_store_box_width * 5) * i_page;
